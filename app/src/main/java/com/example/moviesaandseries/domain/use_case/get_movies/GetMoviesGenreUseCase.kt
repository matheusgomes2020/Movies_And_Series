...
class GetMoviesGenreUseCase @Inject constructor(
    ...
    operator fun invoke( pageNumber: String, genreId: String ): Flow<Resource<List<Movie>>> = flow {

        try {
            emit(Resource.Loading<List<Movie>>())
            val movies = repository.getMoviesGenre( pageNumber, genreId ).map { it.toMovie() }
